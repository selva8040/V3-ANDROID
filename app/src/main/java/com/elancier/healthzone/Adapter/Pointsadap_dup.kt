...
import com.elancier.healthzone.New_wallet_List
...
class Pointsadap_dup(private val mRecyclerViewItems2: List<Any>, private val context: Context, private val listener: OnItemClickListener) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
            ...
            ITEM_CONTENT_VIEW_TYPE -> {
                ...
                holder.remark.setOnClickListener {
                    ...
                    if(holder.remark.text.toString()=="Approved") {
                        (context as Point_List).clikffed(item.id, item.name,item.whome,item.uName)
                    }
                    else if(holder.remark.text.toString()=="Rejected") {
                        (context as Point_List).clikffed_rej(item.id, item.points,item.points,item.uName)
                    }
