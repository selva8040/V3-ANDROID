...
class NSP_Redeem_Wallet : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!utils.loadnspwallet().isEmpty()) {
            val point =
                utils.loadnspwallet().toString().split("\\.".toRegex(), 2).toTypedArray()[0]
            println("point$point")
            try {
                if (!point.isEmpty()) {
       ...
       /* if (!utils.loadbasewallet().isEmpty()) {
            val point =
        ...
        swipeToRefresh.setOnRefreshListener(object : SwipeRefreshLayout.OnRefreshListener {
            override fun onRefresh() {
                ...
                finish()
                    ...
                    startActivity(Intent(this@NSP_Redeem_Wallet, NSP_Redeem_Wallet::class.java))
        ...
        addsalary.setOnClickListener{
            try {
                 update = Dialog(this@NSP_Redeem_Wallet)
        ...
        @SuppressLint("WrongThread")
         override fun doInBackground(vararg params: String?): String? {
            ...
            try {
                ...
                Log.i(
                    "HomePage Input",
                    Appconstants.nsp_activation + "    " + jobj.toString()
                )
                result = con.sendHttpPostjson2(Appconstants.nsp_activation, jobj, "")
            } catch (e: java.lang.Exception) {
                e.printStackTrace()
    ...
    fun clikffed(pos: String, time: String, time1: String, time2: String) {
        try {
            val update = Dialog(this@NSP_Redeem_Wallet)
            update.requestWindowFeature(Window.FEATURE_NO_TITLE)
            update.window!!.setBackgroundDrawable(
                ColorDrawable(Color.WHITE)
            )
